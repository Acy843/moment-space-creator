...
export async function hydrateUserState(u: AuthenticatedMovaUser): Promise<HydratedState> {
  ...
  if (userDoc?.onboardingCompleted) {
    ...
    if (existing.length === 0) {
      const plan = buildDailySchedule(profile ?? localProfileFromDraft({ occupation: DEMO_FALLBACK_OCCUPATION, workStyle: [], constraints: [], breakRhythm: "balanced" }), new Date());
...
export async function startResetFlow(u: AuthenticatedMovaUser | null, resetId: string): Promise<Reset | null> {
  const existing = await loadResetForMutation(u, resetId);
  if (!existing || existing.status !== "scheduled") return null;
  const started: Reset = { ...existing, status: "active", startedAt: nowIso(), updatedAt: nowIso() };
  if (isFirebaseUser(u)) await updateReset(u.uid, resetId, { status: "active", startedAt: started.startedAt });
